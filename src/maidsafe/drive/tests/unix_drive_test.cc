...
namespace {
const std::initializer_list<mode_t> possible_types = {S_IFSOCK, S_IFLNK, S_IFREG, S_IFBLK,
                                                      S_IFDIR,  S_IFCHR, S_IFIFO};

const std::initializer_list<mode_t> supported_types = {S_IFDIR, S_IFREG, S_IFLNK};

const std::initializer_list<mode_t> possible_permissions = {S_IRUSR, S_IWUSR, S_IXUSR, S_IRGRP,
...
bool VerifyPermissions(const std::set<mode_t>& expected, const mode_t actual) {
  ...
  const auto not_has_expected =
      [has_expected](const mode_t not_expected) { return !has_expected(not_expected); };

  return detail::test::VerifyDistinctSets(expected, possible_permissions, has_expected,
                                          not_has_expected);
}
...
TEST(UnixDriveTest, BEH_ModePermissionMask) {
  EXPECT_TRUE(static_cast<mode_t>(boost::filesystem::perms_mask) == detail::ModePermissionMask());
}

TEST(UnixDriveTest, BEH_ToPermssionMode) {
  {
    EXPECT_TRUE(VerifyNonPermissionBits(detail::ToPermissionMode(
        static_cast<detail::MetaData::Permissions>(std::numeric_limits<mode_t>::min()))));
  }
  {
    EXPECT_TRUE(VerifyNonPermissionBits(detail::ToPermissionMode(
        static_cast<detail::MetaData::Permissions>(std::numeric_limits<mode_t>::max()))));
  }
  {
  ...
  {
    const mode_t mode = detail::ToPermissionMode(detail::MetaData::Permissions::owner_all |
                                                 detail::MetaData::Permissions::group_exe |
                                                 detail::MetaData::Permissions::others_write |
                                                 detail::MetaData::Permissions::set_uid_on_exe);
    EXPECT_TRUE(VerifyNonPermissionBits(mode));
    EXPECT_TRUE(VerifyPermissions({S_IRUSR, S_IWUSR, S_IXUSR, S_IXGRP, S_IWOTH, S_ISUID}, mode));
  }
}

TEST(UnixDriveTest, BEH_ToFileType) {
  EXPECT_EQ(detail::MetaData::FileType::directory_file,
            detail::ToFileType(S_IFDIR | detail::ModePermissionMask()));
  EXPECT_EQ(detail::MetaData::FileType::regular_file,
            detail::ToFileType(S_IFREG | detail::ModePermissionMask()));
  EXPECT_EQ(detail::MetaData::FileType::symlink_file,
            detail::ToFileType(S_IFLNK | detail::ModePermissionMask()));

  EXPECT_EQ(detail::MetaData::FileType::status_error,
...
TEST(UnixDriveTest, BEH_ToFileMode) {
  EXPECT_EQ(S_IFDIR, detail::ToFileMode(detail::MetaData::FileType::directory_file));
  EXPECT_EQ(S_IFREG, detail::ToFileMode(detail::MetaData::FileType::regular_file));
  EXPECT_EQ(S_IFLNK, detail::ToFileMode(detail::MetaData::FileType::symlink_file));
}
