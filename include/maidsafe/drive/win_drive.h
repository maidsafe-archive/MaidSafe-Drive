...
template<typename Storage> class CbfsDriveInUserSpace;
...
boost::filesystem::path GetRelativePath(CbFsFileInfo* file_info) {
  ...
  boost::scoped_array<WCHAR>
      file_name(new WCHAR[Global<Storage>::g_cbfs_drive->max_file_path_length()]);
...
template<typename Storage>
void CbfsDriveInUserSpace<Storage>::UnmountDrive(const bptime::time_duration &timeout_before_force) {
...
void CbfsDriveInUserSpace<Storage>::CbFsGetVolumeLabel(CallbackFileSystem* sender,
  ...
  wcsncpy_s(volume_label, Global<Storage>::g_cbfs_drive->drive_name().size() + 1,
            &Global<Storage>::g_cbfs_drive->drive_name().at(0),
...
void CbfsDriveInUserSpace<Storage>::CbFsCreateFile(CallbackFileSystem* sender,
  ...
  FileContext<Storage>* file_context(new FileContext<Storage>(relative_path.filename(), is_directory));
...
void CbfsDriveInUserSpace<Storage>::CbFsOpenFile(CallbackFileSystem* sender,
  ...
  fs::path relative_path(file_name);
  FileContext<Storage>* file_context(new FileContext<Storage>);
      ...
      if (file_context->self_encryptor) {
        if (file_context->self_encryptor->Flush()) {
          if (file_context->content_changed) {
            try {
              Global<Storage>::g_cbfs_drive->UpdateParent(file_context,
...
template<typename Storage>
void CbfsDriveInUserSpace<Storage>::CbFsGetFileInfo(
...
void CbfsDriveInUserSpace<Storage>::CbFsSetAllocationSize(CallbackFileSystem* sender,
  ...
  LOG(kInfo) << "CbFsSetAllocationSize - " << relative_path << " to " << allocation_size
             << " bytes.";
  if (file_info->get_UserContext() != nullptr) {
    FileContext<Storage>* file_context(static_cast<FileContext<Storage>*>(file_info->get_UserContext()));
...
void CbfsDriveInUserSpace<Storage>::CbFsCanFileBeDeleted(CallbackFileSystem* sender,
  ...
  if (Global<Storage>::g_cbfs_drive->CanRemove(relative_path)) {
    *can_be_deleted = true;
  } else {
...
void CbfsDriveInUserSpace<Storage>::CbFsDeleteFile(CallbackFileSystem* sender,
  ...
  std::shared_ptr<FileContext<Storage>> file_context(new FileContext<Storage>);
  try {
    Global<Storage>::g_cbfs_drive->GetMetaData(
          relative_path, *file_context->meta_data.get(), nullptr, nullptr);
...
void CbfsDriveInUserSpace<Storage>::CbFsIsDirectoryEmpty(CallbackFileSystem* sender,
  ...
  try {
    DirectoryListingHandler<Storage>::DirectoryType
        directory(Global<Storage>::g_cbfs_drive->directory_listing_handler_->GetFromPath(file_name));
